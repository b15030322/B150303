...
void calcuMark(double m[4][4],Book boo[],int n); /*求两数据的最高、最低、平均，m数组第一维*//*表示哪项数据，第二维表示最高、最低、平均*/
void sortBoo(Book boo[],int n,int condition);   /*选择法从小到大排序，按condition所规定的条件*/
int searchBoo(Book boo[],int n,Book b,int condition,int f[]);  /*根据条件找数组中与s相等的各元素*//*下标置于f数组中，设f数组是因为查找结果可能不止一条记录*/
int insertBoo(Book boo[],int n,Book b);                   /*向数组中插入一个元素按学号有序*/
int deleteBoo(Book boo[],int n,Book b);                   /*从数组中删除一个指定学号的元素*/
#endif
